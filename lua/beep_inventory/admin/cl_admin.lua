...
BCORE.netstream.Hook("BCORE.Inventory.Admin.SendItemTypes", function(a, b)
    itemTypes = ((type(a) == "table" and b == nil) and a or b) or {}
end)
...
local function RequestInventory(steamid, isModifiers)
    BCORE.netstream.Start("BCORE.Inventory.Admin.RequestPlayerInventory", steamid, isModifiers or false)
end
...
local function FilterTable(searchText, tbl)
    ...
    local function toSearchableString(v)
        ...
        elseif t == "table" then
            ...
            for k, val in pairs(v) do
                str = str .. " " .. toSearchableString(k) .. " " .. toSearchableString(val)
            end
            return str
        else
            return tostring(v)
    ...
    for _, v in ipairs(tbl or {}) do
        if toSearchableString(v):lower():find(searchText, 1, true) then
...
local ACTION_COLORS = {
    duplicate = Color(80,  200, 120),
    delete    = Color(220, 70,  70),
    edit      = Color(90,  160, 240),
    wipe      = Color(240, 150, 40),
    give      = Color(160, 100, 240),
}
local function ActionColor(action)
    return ACTION_COLORS[(action or ""):lower()] or Color(180, 180, 180)
end
...
function BCORE.Inventory:AdminSetCurrentPlayer(steamid) currentplayer = steamid end
function BCORE.Inventory:AdminGetCurrentPlayer()        return currentplayer    end
...
function BCORE.Inventory:OpenAdmin()
    if IsValid(BCORE.Inventory.frame) then BCORE.Inventory.frame:Remove() end
    ...
    function BCORE.Inventory.frame:OnKeyCodePressed(key)
        if BCORE.Inventory.frame:IsVisible() and key == KEY_Inventory then
            ...
            timer.Simple(0.2, function()
                if IsValid(BCORE.Inventory.frame) then BCORE.Inventory.frame:SetVisible(false) end
            end)
        end
    end
    ...
    local C = BCORE.Inventory.colors
    ...
    BCORE.Inventory.actionlogs:FadeHover(Color(C.light.r, C.light.g, C.light.b, 90), 6, 8)
    BCORE.Inventory.actionlogs:On("DoClick", function() BCORE.Inventory:AdminGetAdminActionLogs() end)
    BCORE.Inventory.actionlogs:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
    ...
    BCORE.Inventory.Adminlogs:FadeHover(Color(C.light.r, C.light.g, C.light.b, 90), 6, 8)
    BCORE.Inventory.Adminlogs:On("DoClick", function() BCORE.Inventory:AdminGetAdminlogs() end)
    BCORE.Inventory.Adminlogs:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
        draw.RoundedBox(8, 1, 1, w - 2, h - 2, C.sec)
    ...
    searchhold:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
    ...
    BCORE.Inventory.search:SetTextColor(C.cwhite)
    BCORE.Inventory.search:SetCursorColor(C.cwhite)
    BCORE.Inventory.search.OnChange = function()
        filteredPlayers = FilterTable(BCORE.Inventory.search:GetText():lower(), allPlayers)
        BCORE.Inventory:BuildPlayerPanels(filteredPlayers)
    end
...
function BCORE.Inventory:BuildPlayerPanels(playerList)
    ...
    for _, steamid in ipairs(playerList) do
        ...
        panel:ClearPaint():Background(self.colors.light, 8):On("Paint", function(s, w, h)
            ...
            draw.SimpleText(ply and ply:Nick():gsub("^%l", string.upper) or steamid,
                "BCORE.Inventorys.24", h * 0.9, h / 3, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            draw.SimpleText(ply and "Online" or "Offline",
                "BCORE.Inventorys.22", h * 0.96, h / 1.5,
                ply and Color(0, 255, 0) or Color(255, 0, 0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        end)
        ...
        openInvBtn:FadeHover(Color(self.colors.light.r, self.colors.light.g, self.colors.light.b, 90), 6, 8)
...
OverrideRightClick = function(panels)
    ...
    for i = 1, BCORE.Inventory.config.MaxSlots do
        ...
        if not (panel and panel.HasItem and IsValid(itemPanel) and IsValid(itemPanel.ModelPanel)) then
            continue
        end
        ...
        itemPanel.ModelPanel.DoRightClick = function()
            ...
            -- 🔒 SNAPSHOT ITEM (IMPORTANT FIX)
            local item = table.Copy(itemPanel.Item)
            ...
            menu:AddOption("Duplicate", function()
                local sid = BCORE.Inventory:AdminGetCurrentPlayer()
                if not sid then return end

                BCORE.netstream.Start(
                    "BCORE.Inventory.Admin.RequestAction",
                    sid,
                    "duplicate",
                    item.id,
                    item
            ...
            menu:AddOption("Edit", function()
                BCORE.Inventory:AdminEditItem(item)
            end)
...
function BCORE.Inventory:AdminEditItem(itemData)
    ...
    frame:FadeIn(.5)
    frame:MakePopup()
    frame:ClearPaint():Shadow(50):Background(C.light, 8):On("Paint", function(s, w, h)
    ...
    topbar:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
        draw.RoundedBox(8, 1, 1, w - 2, h - 2, C.sec)
    ...
    for key, value in pairs(itemData.customData or {}) do
        ...
        row:Stick(TOP)
        row:SetTall(frame:GetTall() * .08)
        row:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
            draw.RoundedBox(8, 1, 1, w - 2, h - 2, C.accent)
            BUi.DrawImgur(h * .15, h * .15, h * .7, h * .7, "https://invisibalfan-ui.github.io/bui_images/images/mkp8lur.png", color_white)
        end)

        local entry = BUi.Create("DTextEntry", row)
        entry:Stick(FILL, nil, 40)
        entry:ReadyTextbox()
        entry:SetTall(30)
        entry:SetText(tostring(value or ""))
        entry:SetFont("BCORE.Inventoryb.22")
        entry:SetTextColor(C.cwhite)
        entry:SetCursorColor(C.cwhite)
    ...
    saveBtn:SetText("")
    saveBtn:ClearPaint():Shadow(50):Background(C.light, 8):On("Paint", function(s, w, h)
        draw.RoundedBox(8, 1, 1, w - 2, h - 2, self.colors.bg)
    ...
    saveBtn.DoClick = function()
        ...
        for key, entry in pairs(entries) do
            ...
            updatedCustomData[key] = val:match("^-?%d*%.?%d+$") and tonumber(val) or val
        end
...
function BCORE.Inventory:RenderLogs(logsData)
    if not IsValid(self.GridHolder) then return end
    local C = BCORE.Inventory.colors
    ...
    BCORE.Inventory.back:FadeHover(Color(C.light.r, C.light.g, C.light.b, 90), 6, 8)
    BCORE.Inventory.back:On("DoClick", function()
        BCORE.Inventory:OpenAdmin()
        BCORE.Inventory:BuildPlayerPanels(filteredPlayers)
    end)
    BCORE.Inventory.back:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
        draw.RoundedBox(8, 1, 1, w - 2, h - 2, C.sec)
        draw.SimpleText("Back", "BCORE.Inventorys.24", w / 2, h / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end)

    local ROW_H     = BUi:Scale(72)
    local PILL_W    = BUi:Scale(6)
    local AVATAR_SZ = ROW_H - BUi:Scale(16)
    local PAD       = BUi:Scale(10)

    for _, log in ipairs(logsData or {}) do
        local ply       = player.GetBySteamID64(log.steamid64 or "")
        local adminName = log.admin or (ply and ply:Nick()) or tostring(log.steamid64 or "Unknown")
        local actionStr = tostring(log.action or "unknown"):lower()
        local targetStr = tostring(log.target or "–")
        local timeStr   = os.date("%d %b %Y  %H:%M:%S", log.time or os.time())
        local accentCol = ActionColor(actionStr)

        local panel = BUi.Create("DPanel", self.GridHolder)
        panel:Stick(TOP, 0, 0, 0, 0, 6)
        panel:SetTall(ROW_H)
        panel:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
            draw.RoundedBox(8, 1, 1, w - 2, h - 2, C.sec)
            draw.RoundedBox(4, 2, 4, PILL_W, h - 8, accentCol)

            local textX = PILL_W + PAD + AVATAR_SZ + PAD + PAD
            draw.SimpleText(adminName:gsub("^%l", string.upper), "BCORE.Inventorys.22",
                textX, h * 0.28, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            draw.SimpleText(actionStr:upper() .. "  →  " .. targetStr, "BCORE.Inventorys.20",
                textX, h * 0.62, accentCol, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            draw.SimpleText(timeStr, "BCORE.Inventory.19",
                w - PAD * 2 - BUi:Scale(80), h / 2, Color(180, 180, 180), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
        end)

        local avatar = BUi.Create("AvatarMaterial", panel)
        avatar:SetPos(PILL_W + PAD * 2, (ROW_H - AVATAR_SZ) / 2)
        avatar:SetSize(AVATAR_SZ, AVATAR_SZ)
        avatar:CircleAvatar()
        avatar:SetSteamID64(log.target or log.steamid64 or "")

        local copyBtn = BUi.Create("DButton", panel)
        copyBtn:Stick(RIGHT, 10)
        copyBtn:SetWide(BUi:Scale(80))
        copyBtn:SetText("")
        copyBtn:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
            draw.RoundedBox(8, 1, 1, w - 2, h - 2, C.sec)
            draw.SimpleText("COPY", "BCORE.Inventorys.18", w / 2, h / 2, accentCol, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end)
        copyBtn:FadeHover(Color(C.light.r, C.light.g, C.light.b, 90), 6, 8)
        copyBtn.DoClick = function()
            SetClipboardText(string.format("[%s] Admin: %s | Action: %s | Target: %s",
                timeStr, tostring(log.admin or "Unknown"),
                tostring(log.action or "Unknown"), tostring(log.target or "Unknown")))
        end
    end
end
...
function BCORE.Inventory:AdminLogs(logsData)
    self.GridHolder:Clear()
    logsData = logsData or adminlogs or {}
    self:RenderLogs(logsData)
    BCORE.Inventory.search.OnChange = function()
        self.GridHolder:Clear()
        self:RenderLogs(FilterTable(BCORE.Inventory.search:GetText():lower(), logsData))
    end
end

function BCORE.Inventory:AdminActionLogs(logsData)
    self.GridHolder:Clear()
    logsData = logsData or actionlogs or {}
    self:RenderLogs(logsData)
    BCORE.Inventory.search.OnChange = function()
        self.GridHolder:Clear()
        self:RenderLogs(FilterTable(BCORE.Inventory.search:GetText():lower(), logsData))
...
function BCORE.Inventory:OpenPlayerInventory(steamid)
    ...
    modifiers:FadeHover(Color(C.light.r, C.light.g, C.light.b, 90), 6, 8)
    modifiers:On("DoClick", function(s)
        if s.IsModifierMode then
            s.IsModifierMode = false
            RequestInventory(steamid, false)
        else
            s.IsModifierMode = true
            RequestInventory(steamid, true)
        end
    end)
    modifiers:ClearPaint():Background(self.colors.light, 8):On("Paint", function(s, w, h)
        draw.RoundedBox(8, 1, 1, w - 2, h - 2, self.colors.sec)
        draw.SimpleText(s.IsModifierMode and "Inventory" or "Modifiers",
            "BCORE.Inventorys.24", w / 2, h / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end)
    ...
    back:SetText("")
    back:FadeHover(Color(C.light.r, C.light.g, C.light.b, 90), 6, 8)
    back:On("DoClick", function() BCORE.Inventory:OpenAdmin() end)
    back:ClearPaint():Background(self.colors.light, 8):On("Paint", function(s, w, h)
        draw.RoundedBox(8, 1, 1, w - 2, h - 2, self.colors.sec)
        draw.SimpleText("Back", "BCORE.Inventorys.24", w / 2, h / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end)

    -- Initial load: always through server, no isModifiers flag = regular inventory
    RequestInventory(steamid)
end

-- ─────────────────────────────────────────────
-- ITEM TYPES / GIVE ITEM
-- ─────────────────────────────────────────────
...
function BCORE.Inventory:GiveItem(steamid)
    ...
    giveItemFrame:ClearPaint():Shadow(50):Background(C.light, 8):On("Paint", function(s, w, h)
        draw.RoundedBox(8, 1, 1, w - 2, h - 2, self.colors.bg)
    end)
    ...
    topbar:ClearPaint():Background(C.light, 8):On("Paint", function(s, w, h)
        draw.RoundedBox(8, 1, 1, w - 2, h - 2, C.sec)
        local ply  = player.GetBySteamID64(steamid)
        local nick = ply and BUi.Truncate(ply:Nick(), 10) or BUi.Truncate(steamid, 10)
        draw.SimpleText("Give Item to " .. nick, "BCORE.Inventorys.24", 10, h / 2, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end)
...
concommand.Add("beep_Inventory_admin_open", function()
